...
func find_paths(chart map[string]Cave, path []string, from_cave string) [][]string {
	new_paths := [][]string{}
	...
	for _, c := range chart[from_cave].out {
		if c == "end" {
			...
			continue
		} else if !contains(path, c) || chart[c].is_big {
			new_paths = append(new_paths, find_paths(chart, append(path, c), c)...)
		}
	}
...
func main() {
	...
	fmt.Println(len(find_paths(chart, []string{}, "start")))
}
