...
func parse_bit_string(bit_string []uint64) (Packet, int) {
	...
	if t == 4 {
		...
		switch t {
		case 0:
			for _, p := range new_packet.packets {
				new_packet.value += p.value
			}
		...
		case 2:
			new_packet.value = new_packet.packets[0].value
			for _, p := range new_packet.packets {
				if new_packet.value > p.value {
					new_packet.value = p.value
				}
			}
		...
		case 5:
			if new_packet.packets[0].value > new_packet.packets[1].value {
				new_packet.value = uint64(1)
			}
