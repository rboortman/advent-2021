...
func main() {
	...
	for scanner.Scan() {
		...
		_, p := parse_calculation(line)
		calculations = append(calculations, p)
	}

	result := calculations[0]
	calculations = calculations[1:]
	for _, p := range calculations {
		result = snail_math(result, p)
	}

	// fmt.Println(calculations)
	// fmt.Println(print_pair(result))
	fmt.Println(calc_math(result))
}
